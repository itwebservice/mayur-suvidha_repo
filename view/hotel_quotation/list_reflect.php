...
$array_s = array();
	...
	while($row_quotation = mysql_fetch_assoc($sq_quotation)){
		...
		$temp_arr = array(
			...
			number_format($cost_details['total_amount'],2),
			$emp_name,
			...
			<button class="btn btn-info btn-sm" onclick="send_mail()" title="Send Email" data-toggle="tooltip"><i class="fa fa-envelope-o"></i></button>

			<button class="btn btn-warning btn-sm" onclick="quotation_clone('.$row_quotation['quotation_id'].')" title="Clone Quotation" data-toggle="tooltip"><i class="fa fa-files-o"></i></button>

			'.$whatsapp_show.' 

			<form  style="display:inline-block" action="update/index.php" target="_blank" id="frm_booking_'.$count.'" method="POST">
			...
			<form  style="display:inline-block" action="quotation_view.php" target="_blank" id="frm_booking_view_'.$count.'" method="GET">
				<input  style="display:inline-block" type="hidden" id="quotation_id" name="quotation_id" value="'.$row_quotation['quotation_id'].'">
