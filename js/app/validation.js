...
function foo(tableID, quot_table_id) {
	...
	if (tableID == 'dynamic_table_list_p_' + quot_table_id) {
		...
		row.cells[6].childNodes[0].setAttribute('onclick', 'add_itinerary("dest_name","special_attaraction'+foo.counter+'","day_program'+ foo.counter+'","overnight_stay'+ foo.counter+'")');
		if (row.cells[7]) {
	...
	if (tableID == 'package_program_list') {
		...
		for (var i = row.cells[6].childNodes[0].attributes.length; i-- > 0;)
			row.cells[6].childNodes[0].removeAttribute(row.cells[6].childNodes[0].attributes[i]);
		row.cells[6].childNodes[0].setAttribute('id', 'add_itinerary' + foo.counter);
		row.cells[6].childNodes[0].setAttribute('onclick', 'add_itinerary("dest_name2","special_attaraction'+foo.counter+'","day_program'+ foo.counter+'","overnight_stay'+ foo.counter+'")');
		if (row.cells[7]) {
			$(row.cells[7]).addClass('hidden');
		}
	}
